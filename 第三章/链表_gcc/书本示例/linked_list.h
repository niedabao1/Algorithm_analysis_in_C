...
List CreateList(int num);//创建节点
 ...
 void Insert(int InsertNum,List L ,Position P );//在指定位置插入数字
...
List CreateList(int X)
{
  ...
  List end = malloc(sizeof(struct Node));
  node->Num = X;
  node->Next = end;
  end->Next =NULL;
 return node;
...
int IsLast(Position P,List L)//p是当前位置
{
 ...
 void Insert(int InsertNum,List L ,Position P )
 {
   Position P1  = malloc(sizeof(struct Node));//申请空间
   P1->Num = InsertNum;
   P1->Next = P->Next;
   P->Next = P1;
 }
